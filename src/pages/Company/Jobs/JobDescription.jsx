...
import { controller, getHiringDataById, getUserProfileById } from "../../../services/APIConfig";
...
const JobDescription = () => {
    ...
    function handleJobApplied(data) {
        setHiring((prev) => ({ ...prev, hiring: true }));
    }
    const UserDataPost = () => {
    ...
    formattedDuration === "one" ? (formattedDuration = "1 Month") : (formattedDuration = `${hiring?.detailFound?.duration} Months`);
    ...
    useEffect(() => {
        if (Object.keys(hiring).length !== 0) {
            document.getElementById("quill-job-description").innerHTML = hiring?.detailFound?.description;
        }
    }, [hiring]);
